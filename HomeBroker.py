...
class Bond_Analisys(All_bonds):    
    ...
    def Save_to_Excel(self, df):
        sav = input(str('Nome do Excel:'))
        self.df = df
        return df.to_excel(sav+'.xlsx')
    ...
    def Setor(self):
        fig2 = go.Figure(go.Sunburst(
                labels = self.df_setor['Setor'],
                parents = self.df_setor['Papel'],
                domain=dict(column=1),
                maxdepth=2,
                insidetextorientation='radial'))
        fig2.update_layout(uniformtext=dict(minsize=10, mode='hide'))
        fig2.show()
"""Realizar para índices, ações americanas e moedas"""
"""Lembrar de desativar o zoom nos gráficos em barra"""
